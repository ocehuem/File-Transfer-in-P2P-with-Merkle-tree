...
class Peer:
    ...
    def _handle_file_request(self, client_sock, message):
        ...
        from utils import get_file_hash  # to ensure hash check compatibility
        ...
        if file_hash in self.available_files:
            ...
            try:
                ...
                offset = ack_message.content.get('offset', 0)
                ...
                with open(filepath, 'rb') as f:
                    ...
                    while bytes_sent < filesize:
                        ...
                        if not chunk:
                            break
                        encrypted = xor_encrypt_decrypt(chunk)
                        client_sock.sendall(encrypted)
                        bytes_sent += len(chunk)
                print(f"Sent file {filename} from byte {offset} to {message.sender}")
            except Exception as e:
    ...
    def download_file(self, file_hash, peer_id, destination=None):
        ...
        try:
            ...
            if os.path.exists(filepath):
                ...
                if received == filesize:
                    ...
                    if local_hash == file_hash:
                        print("File already fully downloaded and verified.")
                        sock.close()
                        return True
                    else:
                        print("File size matches, but integrity check failed! Redownloading from start.")
                        received = 0  # Restart download
                        mode = 'wb'
                else:
                    print(f"Resuming download from byte {received}")
                    mode = 'ab'

            # Send FILE_DATA_REQUEST with offset for resume
            ...
            if final_hash != file_hash:
                print(f"Hash mismatch! Expected {file_hash}, got {final_hash}")
                return False
...
class PeerDiscovery:
    ...
    def _listen_loop(self):
        ...
        # Track already printed peers
        if not hasattr(self, 'printed_peers'):
            self.printed_peers = set()
                ...
                try:
                    ...
                    if message.type == MessageType.PEER_ANNOUNCE and message.sender != self.peer_id:
                        ...
                        if addr[0] not in ('0.0.0.0', '127.0.0.1'):
                            ...
                            # ✅ Print only if this peer hasn't been printed before
                            if peer_id not in self.printed_peers:
                                print(f"Discovered peer: {peer_id} at {addr[0]}:{peer_port}")
                                self.printed_peers.add(peer_id)
